...
const showActiveLabel = ref(true)

let labelTimer: ReturnType<typeof setTimeout> | null = null

const showSectionLabel = () => {
    showActiveLabel.value = true
...
<template>
    <nav class="sticky top-0 z-50 w-full h-14 p-4 backdrop-blur-md">
        <div class="md:flex gap-8 justify-center mx-auto max-w-7xl items-center px-4 md:px-8">
    ...
    <div class="fixed right-4 top-1/2 z-50 flex -translate-y-1/2 flex-col items-end gap-4 md:hidden"
    ...
    >
        ...
        >
            <!-- Section label -->
            <span
                class="pointer-events-none absolute right-7
                       whitespace-nowrap rounded-md
                       border border-white/10
                       bg-[#0a0a12]/90 px-3 py-1.5
                       text-xs text-white
            ...
            <!-- Dot -->
            <span
                class="block rounded-full border transition-all
                       duration-300"
                :class="
                    activeSection === item.id
                        ? 'h-3 w-3 border-white bg-white shadow-[0_0_10px_rgba(255,255,255,0.7)]'
                        : 'h-2 w-2 border-gray-500 bg-gray-500/60 group-hover:h-3 group-hover:w-3 group-hover:border-white group-hover:bg-white'
                "
            ></span>
        </a>
    </div>
